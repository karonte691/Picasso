...
namespace Picasso::Engine::Render::Core::Drivers::Vulkan
{
    bool VulkanSwapChainManager::Create(DriverContext *context, u_int32_t width, u_int32_t height, std::shared_ptr<VulkanDevice> m_device)
    {
        ...
        m_imageIndex = std::make_shared<u_int32_t>(0);

        return this->_createSwapChain(context, width, height);
    }

    bool VulkanSwapChainManager::Recreate(DriverContext *context, std::shared_ptr<VulkanDevice> m_device)
    {
        if (context->recreateSwapChain)
        {
            Picasso::Engine::Logger::Logger::Error("Already recreating the swapchain. Booting..");
            return false;
        }

        if (context->frameBufferHeight == 0 || context->frameBufferWidth == 0)
        {
            Picasso::Engine::Logger::Logger::Error("Frame buffer dimension are invalid. Booting...");
            return false;
        }
        ...
        {
            Picasso::Engine::Logger::Logger::Error("Could not detect the depth format. Booting...");
            return false;
        }

        this->Destroy(context);

        return this->Create(context, context->frameBufferWidth, context->frameBufferHeight, m_device);
    }
    ...
    bool VulkanSwapChainManager::FetchNextImageIndex(DriverContext *context, u_int64_t timeout, VkSemaphore imageSemaphore, VkFence fences)
    {
        VkResult acqImageResult = vkAcquireNextImageKHR(context->devices.logicalDevice,
                                                        ...
                                                        timeout,
                                                        imageSemaphore,
                                                        fences == 0 ? VK_NULL_HANDLE : fences,
                                                        m_imageIndex.get());
    ...
    void VulkanSwapChainManager::Present(DriverContext *context, VkQueue graphicsQueue, VkQueue presentQueue, VkSemaphore renderComplete)
    {
